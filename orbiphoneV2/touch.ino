void doSensorReadFast() {
  ...
  if (1) { //direct sensor reading version
    ...
    ampl[0] = fmap(value[0], bufferSlow0.getMin(), bufferSlow0.getMax() + 1, lowerThreshold, 1.);

    //value = touchRead(sensor[1]);
    bufferFast1.addValue(value[1]);
    ampl[1] = fmap(value[1], bufferSlow1.getMin(), bufferSlow1.getMax() + 1, lowerThreshold, 1.);

    //value = touchRead(sensor[2]);
    bufferFast2.addValue(value[2]);
    ampl[2] = fmap(value[2], bufferSlow2.getMin(), bufferSlow2.getMax() + 1, lowerThreshold, 1.);

    //value = touchRead(sensor[3]);
    bufferFast3.addValue(value[3]);
    ampl[3] = fmap(value[3], bufferSlow3.getMin(), bufferSlow3.getMax() + 1, lowerThreshold, 1.);

    //value = touchRead(sensor[4]);
    bufferFast4.addValue(value[4]);
    ampl[4] = fmap(value[4], bufferSlow4.getMin(), bufferSlow4.getMax() + 1, lowerThreshold, 1.);//some weird bug makes the slow min bug

    //value = touchRead(sensor[5]);
    bufferFast5.addValue(value[5]);
    ampl[5] = fmap(value[5], bufferSlow5.getMin(), bufferSlow5.getMax() + 1, lowerThreshold, 1.);

    //value = touchRead(sensor[6]);
    bufferFast6.addValue(value[6]);
    ampl[6] = fmap(value[6], bufferSlow6.getMin(), bufferSlow6.getMax() + 1, lowerThreshold, 1.);

    //value = touchRead(sensor[7]);
    bufferFast7.addValue(value[7]);
    ampl[7] = fmap(value[7], bufferSlow7.getMin(), bufferSlow7.getMax() + 1, lowerThreshold, 1.);

    //value = touchRead(sensor[8]);
    bufferFast8.addValue(value[8]);
    ampl[8] = fmap(value[8], bufferSlow8.getMin(), bufferSlow8.getMax() + 1, lowerThreshold, 1.);

    //value = touchRead(sensor[9]);
    bufferFast9.addValue(value[9]);
    ampl[9] = fmap(value[9], bufferSlow9.getMin(), bufferSlow9.getMax() + 1, lowerThreshold, 1.);

    //value = touchRead(sensor[10]);
    bufferFast10.addValue(value[10]);
    ampl[10] = fmap(value[10], bufferSlow10.getMin(), bufferSlow10.getMax() + 1, lowerThreshold, 1.);

    //value = touchRead(sensor[11]);
    bufferFast11.addValue(value[11]);
    ampl[11] = fmap(value[11], bufferSlow11.getMin(), bufferSlow11.getMax() + 1, lowerThreshold, 1.);
  }
